...
- **Staff auth**: PIN-based (not Supabase Auth), verified via Edge Function `staff-list-orders` (service role key), cached in `sessionStorage` (`bbStaffPin`). PINs stored as bcrypt hash (pgcrypto `crypt()`/`gen_salt('bf')` via `staff_pins_hash` trigger). Admin dashboard cannot reveal existing PINs — only set new ones. Comparison happens only inside `verify_staff_pin()` (service_role only). **When adding a SECURITY DEFINER function, `REVOKE EXECUTE ... FROM PUBLIC` before granting** — Postgres grants EXECUTE to PUBLIC on every new function and `anon` inherits it.
- **Rate limiting**: counted in the `rate_limits` table via `rate_limit_hit()` (service_role only), **not** in a module-level `Map` — Supabase discards the V8 isolate around requests, so in-process counters never accumulate. Two budgets per IP: 60 req/min and 5 wrong PINs/min. Fails open. Migration order: `security-audit.sql` → `pin-hashing.sql` → `security-fixes-2.sql` → `rate-limiting.sql` → `security-fixes-3.sql`.
