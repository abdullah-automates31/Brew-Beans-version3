...
- **Rate limiting**: counted in the `rate_limits` table via `rate_limit_hit()` (service_role only), **not** in a module-level `Map` — Supabase discards the V8 isolate around requests, so in-process counters never accumulate. Two budgets per IP: 60 req/min and 5 wrong PINs/min. Fails open. Migration order: `security-audit.sql` → `pin-hashing.sql` → `security-fixes-2.sql` → `rate-limiting.sql`.
